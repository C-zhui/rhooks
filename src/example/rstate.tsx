...
const ProjectModel = createModel({
  ...
  hook({ state, setState, id, thisInstance }) {
    ...
    const completionRate =
      tasks.length > 0
        ? tasks.filter((task) => task.getHookState().isCompleted).length /
          tasks.length
        : 0;
    ...
    const stats = {
      ...
      completedTasks: tasks.filter((task) => task.getHookState().isCompleted)
        .length,
      inProgressTasks: tasks.filter(
        (task) => task.getHookState().status === "inProgress",
      ).length,
      todoTasks: tasks.filter((task) => task.getHookState().status === "todo")
        .length,
      highPriorityTasks: tasks.filter(
        (task) => task.getState().priority === "high",
      ).length,
    };
...
const initTasks = async () => {
  ...
  await defaultProject
    .getHookState()
    .addTask("完成项目规划", "制定项目的详细计划和时间表", "high");

  console.log("step 2");
  await defaultProject
    .getHookState()
    .addTask("设计用户界面", "创建项目的用户界面设计", "medium");

  console.log("step 3");
  await defaultProject
    .getHookState()
    .addTask("实现核心功能", "开发项目的核心功能模块", "high");
};
                    ...
                    >
                      ...
                      >
                        <select
                          ...
                          onChange={(e) =>
                            taskState.setStatus(e.target.value as TaskStatus)
                          }
                          style={{
                            padding: "5px",
                            borderRadius: "3px",
                        ...
                        <button
                          onClick={() =>
                            project.getHookState().removeTask(task.id)
                          }
                          style={{
                            padding: "5px 10px",
