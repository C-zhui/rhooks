...
function sealSubject(subject: Subject<any> & { [SYMBOL_SEAL]?: boolean }) {
  ...
  const warn = (_words: string) => {
    console.warn("sealed subject", subject, _words);
  };
...
export function createModel<
  ...
  P = void,
  Shared = void,
>(option: ModelOption<S, I, E, P, Shared>) {
  ...
  function createInstance(params: P) {
    ...
    const emit = proxyKeyCreator(
      (k) => (evtData: any) => events[k].next(evtData),
    );
    const actions = proxyKeyCreator((_k) => new Subject());
    const dispatch = proxyKeyCreator(
      (k) => (actData: any) => actions[k].next(actData),
    );
    const states = proxyKeyCreator(
      (k) => new BehaviorSubject((state as any)[k]),
    );
    const asyncStates = proxyKeyCreator(
      (k) => new BehaviorSubject((state as any)[k]),
    );
    ...
    const innerApi: InnerApi<S, I, E> = {
      ...
      setState: (partialValue: Partial<S>): string[] => {
        const old: any = state || {};
        ...
        Object.entries(partialValue).forEach(([k, v]) => {
          if (old[k] !== v) {
            ...
            nextTickTaskQueue.push(() => {
              const propSubject = (asyncStates as Record<string, Subject<any>>)[
                k
  ...
  return createInstance as any as Creator;
}
...
export function useModelState<S extends object, R = S>(
  ...
  const obs = useMemo(
    () =>
      modelInstance
        ? modelInstance.asyncFullState.pipe(
            map(selector),
